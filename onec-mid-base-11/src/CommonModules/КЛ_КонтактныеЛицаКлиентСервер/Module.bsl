...
Процедура ПерерасчетТаблицТоварУслуга(Таблица, СогласованнаяСкидка) Экспорт 
	...
	СуммаПроцентовБольше100 = 0;
	
	Для Каждого Строка Из Таблица Цикл 
		
		СкидкиТабЧасть = Строка.Скидка / 100;
		СогСкидка = СогласованнаяСкидка / 100;
		ОбщаяСкидка = СкидкиТабЧасть + СогСкидка;
		Если ОбщаяСкидка <= 1 Тогда
				ОбщаяСкидка = 1 - ОбщаяСкидка;
				Строка.Сумма = Строка.Цена * Строка.Количество * ОбщаяСкидка;
		Иначе
				СуммаПроцентовБольше100 = СуммаПроцентовБольше100 + 1;
				Строка.Сумма = 0;
	...
	Если СуммаПроцентовБольше100 > 0 Тогда
		Сообщить("Общая скидка больше 100 %");
	КонецЕсли;
